...
export class ParametroController {
    ...
    @ApiResponse({ status: 201, description: 'Parámetro creado correctamente.', type: ParametroEntity})
    ...
    @Post()
    creaarRegistro(@Body() crearParametroDto: CrearParametroDto): Promise<ParametroEntity> {
    ...
    @ApiResponse({ status: 201, description: 'Parámetro actualizado correctamente.', type: ParametroEntity})
    @ApiResponse({ status: 400, description: 'Bad Request: Verifique los datos de entrada' })
    @ApiResponse({ status: 401, description: 'Unauthorized: No tiene permisos para realizar esta acción' })
    @ApiResponse({ status: 403, description: 'Forbidden: Verifique que el token de autenticación sea válido y que no halla expirado.' })
    @ApiResponse({ status: 404, description: 'Not Found: El parámetro no existe.' })
